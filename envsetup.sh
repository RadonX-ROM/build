...
"rnxremote: Add a git remote for matching RADONX repository"
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^rnx_") ; then
       RADONX_BUILD=$(echo -n $1 | sed -e 's/^rnx_//g')
       export BUILD_NUMBER=$( (date +%s%N ; echo $RADONX_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       RADONX_BUILD=
    fi
    export RADONX_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${RADONX_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the RADONX github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var RADONX_VERSION)
    ...
    if (adb shell getprop ro.rnx.device | grep -q "$RADONX_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $RADONX_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.rnx.device | grep -q "$RADONX_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $RADONX_FIXUP_COMMON_OUT ]; then
